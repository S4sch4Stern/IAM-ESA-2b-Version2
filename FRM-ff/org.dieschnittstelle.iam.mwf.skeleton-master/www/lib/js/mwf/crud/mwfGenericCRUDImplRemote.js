...
function GenericCRUDImplRemote(etype) {
  ...
  this.create = function (item, callback) {
    return new Promise((resolve, reject) => {
      ...
      xhr.create(
        baseUrl.call(this),
        mwfUtils.createPersistableClone(item),
        (result) => {
          if (callback) {
            callback(result);
          }
  ...
  this.update = function (id, update, callback) {
    return new Promise((resolve, reject) => {
      ...
      xhr.update(
        baseUrl.call(this) + id,
        mwfUtils.createPersistableClone(update),
        (result) => {
          if (callback) {
            callback(result);
          }
      ...
      xhrq.onreadystatechange = function () {
        if (xhrq.readyState == 4) {
          if (xhrq.status == 200) {
            ...
            console.log(
              "persistMediaContent(): formdata has been processed successfully: Got: " +
                formDataResponse
            );
            ...
            if (!mediaPath) {
              console.error(
                "persistMediaContent(): formdata response does not contain attribute " +
                  attr +
                  ". Somethins seems to be wrong..."
              );
              if (callback) {
                callback();
              }
              ...
              console.log(
                "persistMediaContent(): media path from formdata is: " +
                  mediaPath +
                  ". Re-setting it on entity..."
              );
              ...
              if (responseObj.contentType) {
                ...
                console.warn(
                  "persistMediaContent(): no content type specified for uploaded content!"
                );
              }
              if (callback) {
                callback(entity);
            ...
            console.error(
              "persistMediaContent(): got an error on sending formdata! Status is: " +
                xhrq.status
            );
            if (callback) {
              callback();
