...
function hideMenusAndDialogs() {
  ...
  if (dialog) {
    ...
    if (dialogCtrl) {
      console.log(
        "hideMenusAndDialogs(): open dialog " +
          dialogid +
          " uses an own controller. Invoke hideDialog() on the controller..."
      );
      ...
      console.log(
        "hideMenusAndDialogs(): open dialog " +
          dialogid +
          " does not use a controller. Just change styling..."
      );
      dialog.classList.remove("mwf-shown");
      dialog.classList.add("mwf-hidden");
      dialog.onclick = null;
      console.log(
        "REFACVIEWS: adding transitionend for hiding menus and dialogs: ",
        this.dialog
      );
...
function addRemoveOnFadedToElement(el) {
  if (el) {
    ...
    const removeOnFaded = () => {
      ...
      if (el.parentNode) {
        console.log(
          "REFACVIEWS: transitionend(): remove element from parent: ",
          el
        );
        el.parentNode.removeChild(el);
      } else {
        console.log(
          "REFACVIEWS: transitionend(): remove element from parent not possible as parent has not been set"
        );
      }
    };
...
function removeRemoveOnFadedFromElement(el) {
  ...
  if (removeOnFadedListener) {
    console.info(
      "REFACVIEWS: removeRemoveOnFadedFromElement(): remove transitionend listener from element without listener having been run (which will be the case if a view is obsolete):",
      el
    );
    el.removeEventListener("transitionend", removeOnFadedListener);
  } else {
    console.warn(
      "REFACVIEWS: removeRemoveOnFadedFromElement(): no transitionend listener specified on element. Clarify why function has been called.",
      el
    );
  }
}
...
function touchEnableOffspring(node) {
  ...
  var touchEnabled = node.querySelectorAll(
    "input[type=submit], button, .mwf-imgbutton, .mwf-touchfeedback, .mwf-menu-item"
  );
  ...
  for (i = 0; i < touchEnabled.length; i++) {
    console.log(
      "touch-enable element: " +
        touchEnabled[i] +
        ", with classes: " +
        touchEnabled[i].getAttribute("class")
    );
...
function getListviewAdapter(listviewid) {
  ...
  if (arguments.length == 0 || !listviewid) {
    if (Object.keys(this.listviewAdapters).length == 1) {
      ...
      console.error(
        "getListview(): cannot use default for listview. Either no or multiple listviews exist in view " +
          this.root.id
      );
    }
  } else {
    ...
    if (listview) {
      ...
      console.error(
        "getListview(): listview wih id " +
          listviewid +
          " does not seem to exist in view " +
          this.root.id
      );
    }
  }
...
function applyDatabinding(root, body, data) {
  ...
  console.log(
    "applyDatabinding(): using template body and root children: " + body,
  ...
  console.log(
    "applyDatabinding(): after template application children of root are ",
    root.children.length,
    root
  );
  ...
  console.log(
    "applyDatabinding(): after touch enablement, children of root are ",
    root.children.length,
    root
  );
}
...
function ApplicationState() {
  ...
  function reattachEmbeddedControllers(vc) {
    ...
    for (ctrlid in embeddedViewControllers) {
      ...
      if (ctrl.mwfAttaching) {
        console.log(
          "attaching/detaching embeddable view controller: " + ctrlid
        );
        ctrl.detachFromView();
  ...
  this.pushViewController = function (vc) {
    console.log(
      "pushing view controller: " +
        vc +
        " onto list of " +
        activeViewControllers.length +
        " active controllers..."
    );
    // we invoke pause on the current last element
    if (activeViewControllers.length != 0) {
      var currentViewVC =
        activeViewControllers[activeViewControllers.length - 1];
  ...
  this.popViewController = function (returnData, returnStatus) {
    ...
    console.log(
      "REFACVIEWS: popViewController(): " +
        vc.root.id +
        ". Will call onpause+onstop",
      vc
    );
            ...
            if (activeViewControllers.length != 0) {
              ...
              if (currentViewVC.lcstatus == CONSTANTS.LIFECYCLE.OBSOLETE) {
                ...
                if (currentViewVC.onReturnFromNextView) {
                  currentViewVC
                    .onReturnFromNextView(vc.root.id, returnData, returnStatus)
                    .then(function (goahead) {
                      if (goahead != false) {
                        console.log(
                          "REFACVIEWS: onReturnFromNextView(): continuing regularly " +
                            currentViewVC.root.id
                        );
                        currentViewVC.onresume().then(function () {});
                      } else {
                        console.info(
                          "REFACVIEWS: onReturnFromNextView(): won't continue display of currentview " +
                            currentViewVC.root.id +
  ...
  this.clearControllers = function (callback) {
    ...
    for (var i = 0; i < totalCount; i++) {
      ...
      if (vc) {
        console.log(
          "clearControllers(): popped view controller: " +
            vc.root.id +
            ". Will call onstop"
        );
        vc.onstop().then(
          function () {
            ...
            if (vc.root && vc.root.parentNode) {
              console.log(
                "clearControllers(): after onstop(), will remove view from dom: " +
                  vc.root.id
              );
              vc.root.parentNode.removeChild(vc.root);
            } else {
              console.log(
                "clearControllers(): after onstop(), view is not attached to a parent: " +
                  vc.root.id
              );
            }
            ...
            if (countdown == 0) {
              console.log(
                "clearControllers(): no active view controllers left. Now call the callback from " +
                  vc.root.id
              );
              callback.call(this, vc.root.id);
            } else {
              console.log(
                "clearControllers(): countdown is greaterorequal than one: " +
                  countdown
              );
            }
          }.bind(this)
  ...
  this.addEmbeddedController = function (vc) {
    console.log(
      "adding top level embedded view controller: " + JSON.stringify(vc)
    );
    ...
    if (vc.root.id) {
      ...
      embeddedViewControllers[vc.root.getAttribute("data-mwf-templatename")] =
        vc;
    } else {
      console.error(
        "addEmbeddedController(): embedded controller neither specifies an id, nor a data-mwf-templatename. How shall it be identified?"
      );
    }
  };
...
function ApplicationResources() {
  ...
  this.getTemplateInstance = function (templatename) {
    ...
    if (!template) {
      console.error(
        "getTemplateInstance(): the template " +
          templatename +
          " does not exist!"
      );
    } else {
      ...
      if (template.classList.contains("mwf-databind")) {
        console.log(
          "template " +
            templatename +
            " uses databinding. Return as root+body segmented object..."
        );
        instance = {
...
function ListviewAdapter(_listview, _controller, _resources) {
  ...
  this.update = function (itemId, update) {
    console.log(
      "update(): " + itemId + ", using update: " + mwfUtils.stringify(update)
    );
    // lookup the itemview
    var selector = ".mwf-listitem[data-mwf-id='" + itemId + "']";
    var itemview = listview.querySelector(selector);
    if (itemview) {
      ...
      if (item) {
        ...
        controller.bindListItemView(
          this.listviewId,
          itemview,
          item,
          resources.getTemplateInstance(listitemTemplateId)
        );
      }
    } else {
  ...
  this.getItemview = function (itemId) {
    ...
    if (!itemview) {
      console.error(
        "getItemview(): no itemview could be obtained for item with id: ",
        itemId
      );
    }
    return itemview;
...
function EventMatcher(group, type, target) {
  if (arguments.length == 4) {
    alert(
      "ERROR: EventMatcher does not accept 4 arguments. Do you intend to use Event?"
    );
  }
...
class ViewController {
  ...
  setViewAndArgs(view, args) {
    ...
    console.log(
      "ViewController.setViewAndArgs() view: " +
        view +
        ", args: " +
        (args ? mwfUtils.stringify(args) : " no args")
    );
  ...
  async prepareActionmenu() {
    ...
    var actionmenuControl = this.root.getElementsByClassName(
      "mwf-actionmenu-control"
    );
  ...
  prepareListviews() {
    ...
    if (listviews.length == 0) {
      console.log(
        "prepareListviews(): view " + this.root.id + " does not use listviews"
      );
    } else if (this.listviewsPrepared) {
      console.warn(
        "prepareListviews(): views seem to have been prepared already for " +
          this.root.id +
          ". Ignore call..."
      );
    } else {
      console.log(
        "prepareListviews(): found " + listviews.length + " listviews"
      );
      ...
      var listviewSelectionListener = function (event) {
        ...
        if (
          document.querySelector(".mwf-listitem-menu.mwf-shown") ||
          document.querySelector(".mwf-sidemenu.mwf-expanded") ||
          document.querySelector(".mwf-actionmenu.mwf-expanded") ||
          document.querySelector(".mwf-dialog.mwf-shown")
        ) {
        ...
        function lookupTarget(currentNode, eventData) {
          ...
          if (currentNode.classList.contains("mwf-listview")) {
            if (!eventData) {
              console.log(
                "got click/touch event on listview, but neither a listitem, nor a listitem menu seems to have been selected"
              );
            } else {
          ...
          // if we have reached the menu control, we need to move upwards because we still need to find out which item is meant
            ...
            currentNode.classList.contains("mwf-listitem-menu-control")
          ) {
            if (!eventData) {
              eventData = {};
            }
        ...
        if (eventData) {
          ...
          // we invoke the itemMenuItemSelected function which will also be called when selecting an action from the action menu
            ...
            this.onListItemMenuItemElementSelected(
              eventData.listitemAction,
              eventData.listitem,
              eventData.listview
            );
          } else {
            this.onListItemElementSelected(
              eventData.listitem,
              eventData.listview
            );
          }
      ...
      for (i = 0; i < listviews.length; i++) {
        ...
        this.listviewAdapters[listviews[i].id] = new ListviewAdapter(
          listviews[i],
          this,
  ...
  onActionmenuItemSelected(item) {
    if (item.classList.contains("mwf-disabled")) {
      console.log(
        "ViewController.onActionmenuItemSelected(): action is disabled!"
      );
    } else {
      ...
      if (item.hasAttribute("data-mwf-targetview")) {
        ...
        console.log(
          "ViewController.onActionmenuItemSelected(): item specifies a targetview: " +
            targetview
        );
        ...
        console.log(
          "ViewController.onActionmenuItemSelected(): item specifies a targetaction: " +
            targetaction
        );
        ...
        alert(
          "Cannot handle actionmenu item with mwf-id: " +
            item.getAttribute("data-mwf-id") +
            "! Remove the item or override onActionmenuItemSelected locally!"
        );
      }
    }
  }
  ...
  async onListItemMenuSelected(listitem, listview) {
    console.log(
      "ViewController.onListItemMenuSelected(): " +
        listitem +
        "." +
        listitem.getAttribute("data-mwf-id") +
        " from " +
        listview
    );
    ...
    if (!listitemMenuId) {
      console.log(
        "onListItemMenuSelected(): no listItemMenuId is specified. If listitem menu selection shall be handled, this function needs to be overridden"
      );
    } else {
      ...
      if (!menuElement.tagName) {
        ...
        if (menuElement.root.classList.contains("mwf-databind")) {
          ...
          var itemObj = this.readFromListview(
            listitem.getAttribute("data-mwf-id")
          );
      ...
      var preparedMenu = await this.onPrepareListitemMenu(
        menuElement,
        listitem,
        listview
      );
      ...
      var listitemMenuItemSelectedListener = function (event) {
        function lookupTarget(node) {
          if (node.classList.contains("mwf-listitem-menu")) {
            console.log(
              "we already reached the root of the menu. Click does not seem to have selected a menu item..."
            );
            return null;
          } /* generalise listitem-action */ else if (
            node.classList.contains("mwf-menu-item") ||
            node.classList.contains("mwf-listitem-action")
          ) {
            return node;
          } else {
  ...
  onListItemElementSelected(listitemEl, listview) {
    console.log(
      "ViewController.onListItemElementSelected(): " +
        listitemEl +
        " from " +
        listview
    );
    ...
    console.log(
      "ViewController.onListItemElementSelected(): itemId is: " + itemId
    );

    var itemObj = this.readFromListview(itemId, listview.id);
    if (!itemObj) {
      console.error(
        "ViewController.onListItemElementSelected(): item with selected id " +
          itemId +
          " not found in list!"
      );
    } else {
      ...
      if (targetaction) {
        console.log(
          "ViewController.onListItemElementSelected(): will call targetaction on view controller: " +
            targetaction
        );
        ...
        console.log(
          "ViewController.onListItemElementSelected(): will open targetview on view controller: " +
            targetview
        );
      ...
      //
        ...
        console.log(
          "ViewController.onListItemElementSelected(): will call onListItemSelected()"
        );
  ...
  onListItemSelected(listitem, listviewid) {
    console.info(
      "ViewController.onListItemSelected(): " +
        listitem +
        " from " +
        listviewid +
        ". Override this function locally in order to implement desired behaviour..."
    );
  }
  ...
  onListItemMenuItemElementSelected(menuitem, listitemelement, listview) {
    console.log(
      "ViewController.onListItemMenuItemElementSelected(): " +
        menuitem +
        " for " +
        listitemelement +
        " from " +
    ...
    if (itemid) {
      var itemObj = this.readFromListview(
        listitemelement.getAttribute("data-mwf-id")
      );
      if (!itemObj) {
        console.warn(
          "ViewController.onListItemMenuItemElementSelected(): Item with id does not seem to be contained in list: " +
            listitemelement.getAttribute("data-mwf-id") +
            ". Either this is an error, or selection must be handled by implementing onListItemMenuItemSelected in view controller"
        );
      ...
      console.warn(
        "ViewController.onListItemMenuItemElementSelected(): no itemid specified for listitem. Handling selection needs to be dealt with by the given view controller implementation: " +
          itemid +
          "/" +
          targetview +
          "/" +
  ...
  onListItemMenuItemSelected(menuitem, itemObj, listview) {
    ...
    if (targetaction) {
      console.log(
        "ViewController.onListItemMenuItemSelected(): will call targetaction on view controller: " +
          targetaction
      );
      // we invoke the function dynamically - TODO: need to check how this can be done more elegantly than using eval
      ...
      console.log(
        "ViewController.onListItemMenuItemSelected(): will open targetview on view controller: " +
          targetview
      );
      // we pass the item as argument using the key "item"
      this.nextView(targetview, { item: itemObj });
      ...
      console.error(
        "ViewController.onListItemMenuItemSelected(): selected menu item neither specifies targetview nor targetaction. Either this is an error, or the item should be dealt with in local view controller implementation."
      );
    }
  }
  ...
  updateActionmenuItems(menu) {
    ...
    for (i = 0; i < menuitemElements.length; i++) {
      ...
      if (currentItem.hasAttribute("data-mwf-id")) {
        console.log(
          "updateActionmenuItems(): found menuitem: " +
            currentItem.getAttribute("data-mwf-id")
        );
        this.actionmenuItems[currentItem.getAttribute("data-mwf-id")] =
          currentItem;
      } else {
        // otherwise we create an anonymous item entry using the counter
        console.log(
          "updateActionmenuItems(): found anonymous menuitem: " + currentItem
        );
        this.actionmenuItems[i] = currentItem;
      }
    }
    console.log(
      "updateActionmenuItems(): items are: " +
        mwfUtils.stringify(this.actionmenuItems)
    );
  }
  ...
  async onpause() {
    ...
    if (this.lcstatus != CONSTANTS.LIFECYCLE.OBSOLETE) {
      console.log(
        "REFACVIEWS: adding transitionend to non obsolete controller with lifecycle: ",
        this.lcstatus,
        this.root
      );
      ...
      console.log(
        "REFACVIEWS: skip adding transitionend to obsolete controller: ",
        this.root
      );
    }
  ...
  async oncreate() {
    ...
    console.log(
      "clearing " + dynvalelements.length + " dynamic value elements"
    );
  ...
  async onresume() {
    ...
    if (this.pendingEventListeners.length == 0) {
      console.log(
        "ViewController.onresume(): no pending event listeners exist"
      );
    } else {
      console.log(
        "ViewController.onresume(): found pending event listeners: " +
          this.pendingEventListeners.length
      );
      ...
      while (this.pendingEventListeners.length > 0) {
        ...
        console.log(
          "ViewController.onresume(): will run pending event listener for: " +
            currentListener.boundEvent.desc()
        );
  ...
  processMaterialElements() {
    if (this.root.getElementsByClassName("mwf-material").length > 0) {
      ...
      this.root.querySelectorAll("fieldset.mwf-material").forEach((fs) => {
        console.log(
          "processMaterialElements(): postprocessing material element: " + fs
        );
      ...
      this.root
        .querySelectorAll("fieldset.mwf-material input")
        .forEach((el) => {
      ...
      console.log(
        "processMaterialElements(): view does not seem to use material elements."
      );
    }
  }
  ...
  addMaterialFeedbackSpanToLegend(fs, input) {
    ...
    if (legends.length != 1) {
      console.log(
        "addMaterialFeedbackSpanToLegend(): no or multiple legends exist. Ignore..."
      );
      return;
    }
  ...
  nextView(viewid, viewargs, asRootView) {
    ...
    if (asRootView) {
      applicationState.clearControllers(
        function () {
          console.log(
            "calling nextView() recursively, without rootView option"
          );
      ...
      if (!nextView) {
        ...
        var nextViewControllerClassname = nextView.getAttribute(
          "data-mwf-viewcontroller"
        );
        console.log(
          "using next view controller: " + nextViewControllerClassname
        );
  ...
  async handleDialogWithController(dialogCtrl, data) {
    console.log(
      "showDialog(): dialog uses view controller. Pass arguments and call onresume."
    );
    ...
    if (dialogCtrl.lcstatus == CONSTANTS.LIFECYCLE.CONSTRUCTED) {
      console.log(
        "showDialog(): dialog view controller needs to be initialised. Will call oncreate()..."
      );
  ...
  async hideDialog() {
    ...
    if (!this.dialog) {
      ...
      if (dialogCtrl) {
        ...
        console.log(
          "hideDialog(): prepare removal on faded for dialog without controller",
          this.dialog
        );
        // REFACVIEWS:
        // we remove the dialog on transitionend
        console.log(
          "REFACVIEWS: adding transitionend for dialog: ",
          this.dialog
        );
  ...
  bindElement(elementid, data, parent) {
    ...
    function isTemplateWrapper(element) {
      ...
      if (
        ...
        (element.classList.length == 0 ||
          (element.classList.contains("mwf-template") &&
            (element.classList.length == 2
              ? element.classList.contains("mwf-databind")
              : element.classList.length == 1)))
      ) {
        console.log(
          "bindElement(): element is a mere wrapper. Will attach it to root elemenent if provided: " +
            parent
        );
        return true;
      }
    ...
    if (typeof elementid == "string") {
      ...
      if (!element) {
        ...
        applyDatabinding(
          parent && isTemplateWrapper(element.root) ? parent : element.root,
          element.body,
          data
        );
        ...
        applyDatabinding(
          parent && isTemplateWrapper(element) ? parent : element,
          element.innerHTML,
          data
        );
      ...
      applyDatabinding(
        parent && isTemplateWrapper(elementid) ? parent : elementid,
        elementid.innerHTML,
        data
      );
  ...
  initialiseListview(items, listviewid) {
    if (!this.listviewsPrepared) {
      console.log(
        "initialiseListview(): listviews have not yet been prepared for " +
          this.root.id +
          ". Prepare them..."
      );
      this.prepareListviews();
  ...
  bindListItemView(viewid, itemview, item, replacement) {
    ...
    applyDatabinding(
      replacement ? replacement.root : itemview.root,
      replacement ? replacement.body : itemview.body,
      item
    );
  ...
  addListener(event, listener, runOnPaused) {
    eventhandling.addListener(event, listener, this, {
      runOnPaused: runOnPaused,
    });
  }
  ...
  runEventListener(listener) {
    ...
    switch (this.lcstatus) {
      case CONSTANTS.LIFECYCLE.SHOWING:
        console.log(
          "runEventListener(): controller is running in the foreground. Execute listener..."
        );
        listener.call();
        break;
      case CONSTANTS.LIFECYCLE.PAUSED:
        ...
        if (
          listener.eventHandlerParams &&
          listener.eventHandlerParams.runOnPaused
        ) {
          console.log(
            "runEventListener(): controller is paused, but listener says runOnPaused. Run it..."
          );
          listener.call();
        } else {
          console.log(
            "runEventListener(): controller is paused. Will add listener to pending listeners..."
          );
      ...
      case CONSTANTS.LIFECYCLE.CREATED:
        console.log(
          "runEventListener(): controller has not been shown so far. There might be something wrong, but we will add listener to pending listeners..."
        );
        break;
      default:
        console.log(
          "runEventListener(): listener will not be handled at the current lifecycle phase: " +
            this.lcstatus
        );
    }
  }
...
class SidemenuViewController extends EmbeddedViewController {
  ...
  async oncreate() {
    ...
    for (i = 0; i < menuItems.length; i++) {
      console.log(
        "setting listener on: " + menuItems[i].getAttribute("data-mwf-id")
      );
  ...
  onMenuItemSelected(item) {
    ...
    console.log(
      "SideMenuViewController.onMenuItemSelected(): id " +
        item.id +
        "/ mwf-id " +
        item.getAttribute("data-mwf-id")
    );
    ...
    if (targetVC) {
      ...
      mwfUtils.showToast(
        "option " + item.getAttribute("data-mwf-id") + " is not supported yet!"
      );
    }
  }
  ...
  attachToView(view) {
    ...
    var mainmenuActionElements = view.getElementsByClassName(
      "mwf-img-sandwich-action"
    );
    console.log(
      "SideMenuViewController: found mainmenu elements: " +
        mainmenuActionElements.length
    );
    if (mainmenuActionElements.length == 0) {
      console.log(
        "view " + view.id + " does not seem to use a mainmenu action"
      );
    } else {
...
function getTopLevelView(id) {
  ...
  console.warn(
    "getTopLevelView(): element with id " +
      id +
      " could not be found. Maybe it is a template, though."
  );
...
function extractTopLevelViews() {
  ...
  document.querySelectorAll(".mwf-dialog").forEach((e) => {
    if (e.id) {
      ...
      console.warn(
        "extractTopLevelViews(): found dialog element without id: ",
        e
      );
    }
  });
  ...
  document.querySelectorAll("body > div").forEach((e) => {
    if (e.id) {
      ...
      if (e.classList.contains("mwf-view-initial")) {
        console.log(
          "extractTopLevelViews(): found initial view: ",
          initialView
        );
        initialView = e;
      }
      ...
      if (e.classList.contains("mwf-view-initial")) {
        console.log(
          "extractTopLevelViews(): element is initial view: ",
          initialView
        );
        initialView = e;
      }
  ...
  console.log(
    "extractTopLevelViews(): found " +
      Object.keys(topLevelViews).length +
      " top level view elements, including dialogs..."
  );
}
...
class Application {
  ...
  async oncreate() {
    ...
    if (!initialView) {
      ...
      console.log(
        "Application.oncreate(): determined initialView: " + this.initialView.id
      );
    }
  }
  ...
  registerCRUD(entitytype, scope, impl) {
    console.log(
      "registerCRUD(): crudops declaration for " +
        entitytype +
        " in scope " +
        scope
    );
  ...
  initialiseCRUD(scope, em) {
    ...
    console.log(
      "initialiseCRUD(): crudops declaration is: " +
        mwfUtils.stringify(this.crudops)
    );
    ...
    for (entitytype in this.crudops) {
      ...
      if (!impl) {
        console.error(
          "initialiseCRUD(): could not find impl for entitytype " +
            entitytype +
            " in scope " +
            scope +
            ".This will not work!"
        ...
        console.log(
          "initialiseCRUD(): initialising impl for entitytype " +
            entitytype +
            " in scope: " +
            scope
        );
  ...
  switchCRUDForType(entitytype, scope, em) {
    ...
    if (!impl) {
      console.warn(
        "switchCRUDForType(): could not find impl for entitytype " +
          entitytype +
          " in scope " +
          scope +
          ". Will not change existing impl"
      );
    } else {
      console.log(
        "switchCRUDForType(): switching impl for entitytype " +
          entitytype +
          " to scope: " +
          scope
      );
...
function onloadApplication() {
  ...
  if (stylingElements.length > 0) {
    console.info(
      "application is running in styling mode. All elements from body will be removed apart from the ones marked as mwf-styling"
    );
    ...
    console.log(
      "adding " + stylingElements.length + " styling elements to body"
    );
    var i, stylingEl;
    ...
    var embeddedTemplates = document.querySelectorAll(
      ".mwf-template .mwf-template"
    );
    if (embeddedTemplates.length == 0) {
      ...
      for (j = 0; j < embeddedTemplates.length; j++) {
        templatesmsg +=
          embeddedTemplates[j].tagName +
          "[" +
          embeddedTemplates[j].getAttribute("data-mwf-templatename") +
          "]: " +
          embeddedTemplates[j].getAttribute("class") +
          "\n";
      }
      alert(
        "ERROR: application contains embedded templates! This will cause trouble!:\n\n " +
          templatesmsg
      );
    }
    ...
    var embeddedComponents = document.querySelectorAll(
      "body > .mwf-view-component"
    );

    console.log(
      "found " + embeddedComponents.length + " top level components!"
    );
    var m,
      currentComponentViewControllerClassname,
      currentComponentViewControllerFunction,
      currentComponentViewController;
    for (m = 0; m < embeddedComponents.length; m++) {
      currentComponentViewControllerClassname = embeddedComponents[
        m
      ].getAttribute("data-mwf-viewcontroller");
      console.log(
        "loading top level embedded view controller: " +
          currentComponentViewControllerClassname
      );
      ...
      currentComponentViewControllerFunction =
        thisapp[currentComponentViewControllerClassname]; //require(currentComponentViewControllerClassname);
      //console.log("currentComponent function: " + currentComponentViewControllerFunction);

      currentComponentViewController =
        new currentComponentViewControllerFunction();
      ...
      if (embeddedComponents[m].classList.contains("mwf-initialise-onload")) {
        currentComponentViewController.oncreate().then(function () {
          applicationState.addEmbeddedController(
            currentComponentViewController
          );
        });
      } else {
    ...
    if (applicationClassname) {
      console.info(
        "onLoadApplication(): using custom application: " + applicationClassname
      );
      // the application will be realised as a singleton!
      applicationObj = thisapp[applicationClassname]; //require(applicationClassname);
    } else {
      console.warn(
        "onLoadApplication(): no custom application is specified. Use default implementation."
      );
    ...
    applicationObj.oncreate().then(function () {
      ...
      var initialViewControllerClassname = initialView.getAttribute(
        "data-mwf-viewcontroller"
      );
      console.log(
        "using initial view controller: " + initialViewControllerClassname
      );

      var initialViewControllerFunction =
        thisapp[initialViewControllerClassname]; //require(initialViewControllerClassname);
      ...
      console.log(
        "after setting view and args, root is: " + initialViewController.root
      );
